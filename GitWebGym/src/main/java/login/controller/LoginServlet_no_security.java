...
public class LoginServlet_no_security extends HttpServlet {
	...
	private MemberBean member_checkAccount(String id, String pw) throws SQLException {
		...
		if (mb != null && pw.equals(mb.getM_password())) {
			return mb;
		}
	...
	private TrainerBean trainer_checkAccount(String id, String pw) throws SQLException {
		...
		if (mb != null && pw.equals(mb.getT_password())) {
			return mb;
		}
...
//		response.addCookie(cookRm);
		...
		if (errMsg.isEmpty()) {
			...
			if (requestURI != null) {
				...
				requestURI = ((context+"/login/login.do").equals(requestURI)) ? request.getContextPath() : requestURI;
